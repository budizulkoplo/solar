...
class CompanyPendingPiutangController extends Controller
{
    /**
     * Halaman daftar pending pembayaran company (in - open)
     */
    public function pendingPembayaranCompany()
    {
        $rekenings = Rekening::forCompany(session('active_company_id'))
            ->orderBy('norek')
            ->get();
        
        return view('transaksi.pending.pembayaranpt', [
            'rekenings' => $rekenings,
            'type' => 'pembayaran'
        ]);
    }
    ...
    /**
     * Halaman daftar piutang company (out - open)
     */
    public function piutangCompany()
    {
        $rekenings = Rekening::forCompany(session('active_company_id'))
            ->orderBy('norek')
            ->get();
        
        return view('transaksi.pending.piutangpt', [
            'rekenings' => $rekenings,
            'type' => 'piutang'
        ]);
    }
    ...
    /**
     * Datatable untuk pending pembayaran company (transaksi masuk dengan status open)
     */
    public function getPendingPembayaranCompany()
    {
        $query = Nota::with([
                'companyUnit:id,company_name',
            ...
            ->where('idcompany', session('active_company_id'))
            ->whereNull('idproject'); // Hanya transaksi company, bukan project
        ...
        return DataTables::eloquent($query)
            ...
            ->addColumn('company_name', function($row) {
                return $row->company ? $row->company->namacompany : '-';
            })
                ...
                if (!empty($search)) {
                    $query->where(function($q) use ($search) {
                        ...
                        ->orWhereHas('company', function($q) use ($search) {
                            $q->where('namacompany', 'like', "%{$search}%");
                        });
                    });
                } else {
    ...
    /**
     * Datatable untuk piutang company (transaksi keluar dengan status open)
     */
    public function getPiutangCompany()
    {
        $query = Nota::with([
                'companyUnit:id,company_name',
                'vendor:id,namavendor',
    ...
    /**
     * Show detail nota company untuk pending/piutang
     */
    public function showCompany($id)
    {
        try {
            ...
            // Validasi bahwa nota ini adalah milik company yang aktif
            if ($nota->idcompany != session('active_company_id')) {
                throw new \Exception("Nota tidak ditemukan atau tidak memiliki akses");
            }
            ...
            return response()->json([
                'success' => true,
                'data' => $nota,
                'company_name' => optional($nota->companyUnit)->company_name,
                'summary' => [
                    'total' => $nota->total,
    ...
    /**
     * Proses pembayaran cicilan/angsuran untuk company
     */
    public function bayarCompany(Request $request, $id)
    {
        DB::beginTransaction();
        try {
            $request->validate([
            ...
            if ($nota->cashflow == 'in') {
                ...
                $keteranganCashflow = "Penerimaan angsuran company nota {$nota->nota_no} - {$request->keterangan}";
            } else {
                // Hutang: kita membayar
                $rekening->saldo -= $jumlahBayar;
                $keteranganCashflow = "Pembayaran angsuran company nota {$nota->nota_no} - {$request->keterangan}";
            }
            ...
            $angsuran = Angsuran::create([
                ...
                'jumlah' => $jumlahBayar,
                'keterangan' => $request->keterangan ?? 'Pembayaran angsuran company'
            ]);
            ...
            // Buat log transaksi
            $logMessage = "Pembayaran angsuran company: Rp " . number_format($jumlahBayar, 0, ',', '.') . 
            ...
            return response()->json([
                'success' => true,
                'message' => 'Pembayaran company berhasil disimpan. Status: ' . ($statusNota ?? 'BELUM LUNAS'),
                'data' => [
            ...
            DB::rollBack();
            \Log::error('Company payment processing error:', [
                'error' => $e->getMessage(),
                'nota_id' => $id,
    ...
    /**
     * Get angsuran history untuk nota company tertentu
     */
    public function getAngsuranHistoryCompany($id)
    {
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil riwayat angsuran company: ' . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Hapus angsuran company (rollback pembayaran)
     */
    public function hapusAngsuranCompany($id)
    {
        DB::beginTransaction();
        try {
            ...
            if (!$user->hasRole('direktur') && !$user->hasRole('keuangan')) {
                return response()->json([
                    'success' => false,
                    'message' => 'Anda tidak memiliki izin untuk menghapus angsuran company'
                ], 403);
            }
            ...
            if ($nota->cashflow == 'in') {
                ...
                $keterangan = "Rollback penerimaan angsuran company nota {$nota->nota_no}";
            } else {
                $rekening->saldo += $angsuran->jumlah;
                $keterangan = "Rollback pembayaran angsuran company nota {$nota->nota_no}";
            }
            ...
            // Buat log
            $logMessage = "Angsuran company dihapus: Rp " . number_format($angsuran->jumlah, 0, ',', '.') . 
            ...
            return response()->json([
                'success' => true,
                'message' => 'Angsuran company berhasil dihapus',
                'data' => [
            ...
            DB::rollBack();
            \Log::error('Delete company angsuran error:', [
                'error' => $e->getMessage(),
                'angsuran_id' => $id
            ]);
    ...
    /**
     * Export laporan pending pembayaran/piutang company
     */
    public function exportReportCompany($type)
    {
        try {
            ...
            $query = Nota::with([
                ...
                ->whereNull('idproject') // Hanya transaksi company
            ...
            foreach ($query as $nota) {
                ...
                $data[] = [
                    ...
                    'Company' => $nota->company ? $nota->company->namacompany : '-',
                    'Total' => $nota->total,
                    'Terbayar' => $totalTerbayar,
                    'Sisa' => $sisa,
