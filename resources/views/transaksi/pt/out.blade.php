<x-app-layout>
    <x-slot name="pagetitle">Transaksi Keluar - PT</x-slot>

    <div class="app-content-header">
        <div class="container-fluid">
            <h3 class="mb-0">Nota Pengeluaran - {{ session('active_company_name') }}</h3>
        </div>
    </div>
                ...
                <div class="row g-0">
                    ...
                    <div class="col-md-9">
                        <form id="frmNota" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="idcompany" value="{{ session('active_company_id') }}">
                            <input type="hidden" name="old_rekening" id="oldRekening">
                            <input type="hidden" name="old_grand_total" id="oldGrandTotal">

                            <div class="modal-header">
                                <h6 class="modal-title" id="modalNotaTitle">Form Nota Keluar - PT</h6>
                            ...
                            <div class="modal-body">
                                <div class="row g-2">
                                    ...
                                    {{-- Bukti Nota (WAJIB untuk transaksi keluar) --}}
                                    <div class="col-12">
                                        <label class="form-label">Bukti Nota <span class="text-danger">*</span></label>
        ...
        <div class="modal-dialog modal-xxl">
            <div class="modal-content">
                <div class="row g-0">
                    ...
                    <div class="col-md-9">
                        <div class="modal-header">
                            <h6 class="modal-title">Detail Nota - PT</h6>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
    ...
    <x-slot name="jscustom">
        ...
        $(document).ready(function() {
            // DataTable untuk PT (OUT)
            let tbNotas = $('#tbNotas').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('transaksi.pt.getdata', 'out') }}",
                columns: [
                    { 
                        data: 'DT_RowIndex', 
                        name: 'DT_RowIndex', 
            ...
            // Generate nomor nota otomatis untuk OUT PT
            function generateNotaNo() {
                ...
                return 'PT-OUT-' + companyId + '-' + tgl + '-' + urut;
            }
            ...
            function resetForm() {
                ...
                $('#modalNotaTitle').text('Form Nota Keluar - PT');
                $('#saldoWarning').hide();
                
                // Wajib bukti nota untuk transaksi keluar
            ...
            function loadUpdateLog(notaId) {
                ...
                $.get(`/transaksi/pt/${notaId}/logs`, function(res) {
            ...
            $(document).on('click', '.view-btn', function() {
                ...
                $.get("/transaksi/pt/" + notaId, function(res) {
                    if (res.success) {
                        ...
                        $('#viewCompany').text(nota.companyUnit ? nota.companyUnit.company_name : '-');
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $.get("/transaksi/pt/" + notaId + "/edit", function(res) {
                    if (res.success) {
                        ...
                        // Isi Vendor untuk transaksi keluar
                        if (nota.vendor_id) {
                        ...
                        // Update modal title
                        $('#modalNotaTitle').text('Edit Nota Keluar - PT');
            ...
            function submitFormData(formElement) {
                ...
                let url = notaId ? "/transaksi/pt/" + notaId + "/out" : "{{ route('transaksi.pt.store', 'out') }}";
