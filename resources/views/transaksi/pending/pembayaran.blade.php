<x-app-layout>
    <x-slot name="pagetitle">Daftar Pending Pembayaran</x-slot>
    ...
    <div class="print-header">
        <h4>Daftar Pending Pembayaran</h4>
    ...
    <div class="app-content-header no-print">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-sm-6">
                    <h3 class="mb-0"><i class="bi bi-clock-history text-warning"></i> Daftar Pending Pembayaran</h3>
                </div>
                <div class="col-sm-6 text-end">
                    <div class="d-flex justify-content-end gap-2">
                        <button class="btn btn-primary btn-sm" onclick="reloadTablePembayaran()">
    ...
    <div class="app-content">
        <div class="container-fluid">
            <div class="card card-pembayaran card-outline">
                <div class="card-header">
                    <h5 class="card-title mb-0">Data Transaksi Masuk</h5>
                </div>
                <div class="card-body">
                    <table id="tblPembayaran" class="table table-sm table-bordered table-striped" style="width: 100%; font-size: small;">
                        ...
                        <tfoot class="table-warning">
                            <tr>
                                <th colspan="6" class="text-end"><strong>TOTAL PENDING PEMBAYARAN</strong></th>
    ...
    <!-- Modal untuk Terima Pembayaran -->
    <div class="modal fade" id="bayarModal" tabindex="-1" aria-labelledby="bayarModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="bayarModalLabel">Terima Pembayaran</h5>
                ...
                <div class="modal-body">
                    <form id="bayarForm">
                        ...
                        <div class="mb-3">
                            <label for="tanggal_bayar" class="form-label">Tanggal Terima <span class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            <label for="jumlah_bayar" class="form-label">Jumlah Terima <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="jumlah_bayar" name="jumlah_bayar" min="1" required>
                            <small class="text-muted" id="sisaText">Sisa Pending: Rp 0</small>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-warning" id="simpanBayar">Terima Pembayaran</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="detailModal" tabindex="-1" aria-labelledby="detailModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="detailModalLabel">Detail Pending Pembayaran</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <table class="table table-sm table-borderless">
                                    ...
                                    <tr>
                                        <th>Sisa Pending</th>
                                        <td id="detailSisa" class="fw-bold text-warning"></td>
                                    </tr>
                                </table>
                            </div>
                        </div>
    ...
    <x-slot name="jscustom">
        ...
        <script>
            ...
            function reloadTablePembayaran() {
                if (tablePembayaran) {
                    tablePembayaran.ajax.reload(null, false);
                }
            }

            function showDetailPembayaran(notaId) {
                $('#detailLoading').show();
                $('#detailContent').hide();
            ...
            function openBayarModalPembayaran(notaId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#sisaText').text('Sisa Pending: ' + formatRupiah(currentSisa));
            ...
            $(document).ready(function () {
                ...
                // Initialize DataTable for Pembayaran
                tablePembayaran = $('#tblPembayaran').DataTable({
                    processing: true,
                    serverSide: true,
                    searching: true,
                    paging: true,
                    ...
                    ajax: {
                        url: "{{ route('pending.data.pembayaran') }}",
                        type: 'GET',
                        dataSrc: function(json) {
                            ...
                            console.log('DEBUG - Calculated totals:', globalTotals);
                    ...
                    columns: [
                        ...
                        { 
                            ...
                            render: function(data, type, row) {
                                ...
                                if (!notaId) {
                                    console.error('DEBUG - No ID found for row:', row);
                                ...
                                return `
                                    <div class="btn-group btn-group-sm" role="group">
                                        <button class="btn btn-info btn-action" 
                                                onclick="showDetailPembayaran(${notaId})"
                                                title="Lihat Detail">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                        <button class="btn btn-success btn-action" 
                                                onclick="openBayarModalPembayaran(${notaId})"
                    ...
                    buttons: [
                        {
                            ...
                            title: 'Daftar Pending Pembayaran',
                            messageTop: 'Tanggal Export: ' + moment().format('DD/MM/YYYY HH:mm'),
                            filename: function() {
                                return 'Pending_Pembayaran_' + moment().format('YYYYMMDD_HHmmss');
                            }
                        },
                        {
                            extend: 'print',
                            ...
                            messageTop: function() {
                                return `<div class="print-header">
                                            <h4 style="text-align: center; margin-bottom: 10px;">Daftar Pending Pembayaran</h4>
                                ...
                                const totals = `
                                    <div style="margin-top: 20px; font-weight: bold; border-top: 2px solid #000; padding-top: 10px;">
                                        <div style="float: right; text-align: right;">
                                            <div>Total Pending: ${formatRupiah(globalTotals.total)}</div>
                    ...
                    language: {
                        emptyTable: "Tidak ada data pending pembayaran",
                    ...
                    drawCallback: function() {
                        console.log('DEBUG - DataTable draw complete, rows:', this.api().rows().count());
                    }
                });
                    ...
                    if (confirm('Apakah Anda yakin ingin menghapus penerimaan pembayaran ini?')) {
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    alert(response.message);
                                    showDetailPembayaran(currentNotaId); // Reload detail modal
                                    tablePembayaran.ajax.reload(); // Reload table
                                } else {
                                    alert('Error: ' + response.message);
                                }
                            },
