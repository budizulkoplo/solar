                                            ...
                                            <td colspan="2">
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="number"  class="form-control form-control-sm text-end" 
                                                           name="ppn" id="ppnAmount" placeholder="Nominal PPN" min="0" value="" step="0.01">
                                                </div>
                                            </td>
    ...
    <x-slot name="jscustom">
        ...
        $(document).ready(function() {
            ...
            let tbNotas = $('#tbNotas').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('transaksi.project.getdata', 'out') }}",
                    type: 'GET',
            ...
            function initAutoNumeric() {
                ...
                if ($('#ppnAmount').length) {
                    try {
                        $('#ppnAmount').attr('type', 'text');
                        if (!$('#ppnAmount').hasClass('auto-numeric-initialized')) {
                            let instance = new AutoNumeric('#ppnAmount', autoNumericOptionsDecimal);
                            autoNumericInstances.push(instance);
                            $('#ppnAmount').addClass('auto-numeric-initialized auto-numeric-input');
                        }
                    } catch (e) {
                        console.error('Error init ppn:', e);
                    }
                }

                if ($('#diskonValue').length) {
                    try {
                        $('#diskonValue').attr('type', 'text');
                        if (!$('#diskonValue').hasClass('auto-numeric-initialized')) {
                            let instance = new AutoNumeric('#diskonValue', autoNumericOptionsDecimal);
                            autoNumericInstances.push(instance);
                            $('#diskonValue').addClass('auto-numeric-initialized auto-numeric-input');
                        }
                    } catch (e) {
                        console.error('Error init diskon:', e);
                    }
                }

                const displayFields = ['#subtotal', '#ppnDisplay', '#diskonDisplay', '#grandTotal'];
                displayFields.forEach(fieldId => {
                    if ($(fieldId).length && !$(fieldId).hasClass('auto-numeric-initialized')) {
                        try {
                            $(fieldId).attr('type', 'text');
                            let instance = new AutoNumeric(fieldId, autoNumericOptionsDisplay);
                            autoNumericInstances.push(instance);
                            $(fieldId).addClass('auto-numeric-initialized');
                        } catch (e) {
                            console.error(`Error init ${fieldId}:`, e);
                        }
                    }
                });
            }
            ...
            function calculateGrandTotal() {
                let subtotal = parseNumber($('#subtotal'));
                let ppnAmount = parseNumber($('#ppnAmount'));
                
                let ppnInstance = AutoNumeric.getAutoNumericElement($('#ppnDisplay')[0]);
                if (ppnInstance) {
                    ppnInstance.set(ppnAmount);
                } else {
                    $('#ppnDisplay').val(formatNumber(ppnAmount));
                }
                
                let diskonAmount = 0;
                let diskonType = $('#diskonType').val();
                let diskonValue = parseNumber($('#diskonValue'));
                ...
                let diskonInstance = AutoNumeric.getAutoNumericElement($('#diskonDisplay')[0]);
                if (diskonInstance) {
                    diskonInstance.set(diskonAmount);
                } else {
                    $('#diskonDisplay').val(formatNumber(diskonAmount));
                }
                ...
                if (grandTotalInstance) {
                    grandTotalInstance.set(grandTotal);
                } else {
            ...
            function updateSaldoDisplay() {
                $('#availableBalance').text(formatNumber(currentSaldo));
            }
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (res.success) {
                            ...
                            $('#viewTotal').text(nota.total ? 'Rp ' + Math.round(nota.total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') : 'Rp 0');
                            ...
                            $('#viewSubtotal').text('Rp ' + Math.round(subtotal).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'));
                            $('#viewPpn').text('Rp ' + Math.round(ppn).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'));
                            $('#viewDiskon').text('Rp ' + Math.round(diskon).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'));
                            $('#viewGrandTotal').text('Rp ' + Math.round(nota.total || 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'));
                                ...
                                nota.transactions.forEach(function(transaction) {
                                    ...
                                    detailHtml += `
                                        <tr>
                                            ...
                                            <td class="text-end">Rp ${Math.round(nominal).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</td>
                                            <td class="text-end">Rp ${Math.round(total).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</td>
                                        </tr>
                                    `;
                                });
                            } else {
            ...
            function submitFormData(formElement) {
                ...
                let subtotal = parseNumber($('#subtotal'));
                let ppnAmount = parseNumber($('#ppnAmount'));
                let diskonAmount = parseNumber($('#diskonDisplay'));
